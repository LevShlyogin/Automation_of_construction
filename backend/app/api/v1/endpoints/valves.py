...
from backend.app.models import Valve, Turbine
from backend.app.schemas import ValveInfo, ValveCreate, TurbineInfo
...
async def update_valve(valve_id: int, valve: ValveInfo, db: Session = Depends(get_db)):
    try:
        ...
        logger.error(f"Ошибка при обновлении клапана: {e}")
...
async def delete_valve(valve_id: int, db: Session = Depends(get_db)):
    try:
        ...
        db.delete(db_valve)
        db.commit()
        return {"message": f"Клапан '{db_valve.name}' успешно удалён"}
    except Exception as e:
        logger.error(f"Ошибка при удалении клапана: {e}")
...
async def get_turbine_by_valve_name(valve_name: str, db: Session = Depends(get_db)):
    try:
        ...
        if not valve or not valve.turbine_id:
            raise HTTPException(status_code=404, detail="Клапан или турбина не найдены")
        ...
        return TurbineInfo(id=turbine.id, name=turbine.name)
    except Exception as e:
        logger.error(f"Ошибка при получении турбины по имени клапана: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Не удалось получить турбину: {e}") 