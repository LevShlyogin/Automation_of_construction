...
turbineName,
		...
		return __request(OpenAPI, {
			method: 'GET',
			url: '/api/turbines/{turbine_name}/valves/',
			path: {
...
valveName,
		...
		return __request(OpenAPI, {
			method: 'GET',
			url: '/api/valves/{valve_name}/turbine',
			path: {
...
valveName,
		...
		return __request(OpenAPI, {
			method: 'GET',
			url: '/api/valves/{valve_name}/results/',
			path: {
				valve_name: valveName
			},
			errors: {
...
export type TDataDiagramsGenerateScheme = {
                requestBody: ValveInfo_Input
 ...
 * HTTPException: Если произошла ошибка при генерации файла.
	 ...
	 * @throws ApiError
	 */
	public static diagramsGenerateScheme(data: TDataDiagramsGenerateScheme): CancelablePromise<any> {
		const {
requestBody,
} = data;
		return __request(OpenAPI, {
			method: 'POST',
			url: '/api/generate_scheme',
			body: requestBody,
			mediaType: 'application/json',
			errors: {
				422: `Validation Error`,
